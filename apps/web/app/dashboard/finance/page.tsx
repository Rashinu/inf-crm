...
export default function FinancePage() {
    ...
    const { data: commissionsData, isLoading: loadingCommissions } = useQuery<any>({
        queryKey: ["finance", "commissions"],
        queryFn: async () => {
            const { data } = await apiClient.get("/finance/commissions");
            return data;
        },
    });
        ...
        <div className="space-y-8">
            ...
            <Tabs defaultValue="overview" className="w-full">
                ...
                <TabsContent value="commissions" className="space-y-8">
                    {loadingCommissions ? (
                        <div className="h-32 flex items-center justify-center text-gray-500">Loading commission data...</div>
                    ) : commissionsData ? (
                        <>
                            <div className="grid gap-6 md:grid-cols-2">
                                <Card className="border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none bg-indigo-600 text-white relative overflow-hidden ring-1 ring-slate-100/10">
                                    ...
                                    <CardContent>
                                        <div className="text-4xl font-black font-outfit tracking-tight text-white mt-2">
                                            ₺{commissionsData.summary.totalSalesRepCommissions.toLocaleString()}
                                        </div>
                                    </CardContent>
                                </Card>

                                <Card className="border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none bg-orange-600 text-white relative overflow-hidden ring-1 ring-slate-100/10">
                                    <div className="absolute -right-4 -top-4 opacity-10">
                                        <TrendingUp size={120} />
                                    </div>
                                    <CardHeader className="pb-2">
                                    ...
                                    <CardContent>
                                        <div className="text-4xl font-black font-outfit tracking-tight text-white mt-2">
                                            ₺{commissionsData.summary.totalInfluencerCommissions.toLocaleString()}
                                        </div>
                                    </CardContent>
                                </Card>
                            </div>
                            ...
                            <Card className="border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none bg-white dark:bg-slate-900 ring-1 ring-slate-100 dark:ring-slate-800">
                                ...
                                <div className="overflow-x-auto">
                                    <Table>
                                        ...
                                        <TableBody>
                                            {commissionsData.items.map((item: any) => (
                                                <TableRow key={item.dealId}>
                                                    <TableCell>
                                                        <div className="font-bold text-slate-900 dark:text-white">{item.dealTitle}</div>
                                                        <div className="text-xs text-slate-500">{item.brandName}</div>
                                                    </TableCell>
                                                    <TableCell>
                                                        <div className="font-bold">₺{item.dealAmountTry.toLocaleString()}</div>
                                                        <div className="text-[10px] text-slate-500">Paid: ₺{item.totalPaidTry.toLocaleString()}</div>
                                                    </TableCell>
                                                    <TableCell>
                                                        <div className="text-sm font-medium text-slate-700 dark:text-slate-300">{item.salesRepName}</div>
                                                        {item.salesRepRate > 0 ? (
                                                            <div className="text-xs text-indigo-600 dark:text-indigo-400 font-bold">
                                                                {item.salesRepRate}% = ₺{item.salesRepCommissionTry.toLocaleString()}
                                                            </div>
                                                        ) : (
                                                    ...
                                                    <TableCell>
                                                        <div className="text-sm font-medium text-slate-700 dark:text-slate-300">{item.influencerName}</div>
                                                        {item.influencerRate > 0 ? (
                                                            <div className="text-xs text-orange-600 dark:text-orange-400 font-bold">
                                                                {item.influencerRate}% = ₺{item.influencerCommissionTry.toLocaleString()}
                                                            </div>
                                                        ) : (
                                                            <div className="text-xs text-slate-400">-</div>
                                                        )}
