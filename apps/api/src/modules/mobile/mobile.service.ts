...
export class MobileService {
  ...
  applyLatePenaltyIfNecessary(
    ...
    if (now.getTime() > deadline.getTime()) {
      return netIncome * 0.9; // 10% penalty
    }
    return netIncome;
  }
  ...
  async getContractsForFlutter(tenantId: string) {
    ...
    return deals.map((deal) => {
      ...
      netIncome = this.applyLatePenaltyIfNecessary(
        netIncome,
        deadline,
        deal.stage,
      );
      ...
      const isLate =
        new Date().getTime() > deadline.getTime() && deal.stage !== 'COMPLETED';
