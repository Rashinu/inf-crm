...
export class BillingService implements OnModuleInit {
    ...
    async getBillingStatus(tenantId: string) {
        ...
        const now = new Date();
        const trialActive = tenant.billingStatus === 'TRIAL' && tenant.trialEndsAt && tenant.trialEndsAt > now;
    ...
    async createCheckoutSession(tenantId: string, payload: { expectedUserCount: number, billingCycle: 'monthly' | 'yearly' }) {
        const tenant = await this.prisma.tenant.findUnique({
            where: { id: tenantId },
        ...
        let finalAmount = payload.billingCycle === 'yearly' ? (monthlyTotal * 12 * 0.8) : monthlyTotal; // 20% discount for yearly
