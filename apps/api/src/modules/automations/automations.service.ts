...
export class AutomationsService {
  ...
  async handleDealStageChanged(payload: {
    ...
    this.logger.log(
      `Received deal stage change event for deal $${payload.dealId}`,
    );
      ...
      if (auto.actionType === 'SEND_EMAIL') {
        ...
        if (email) {
          await this.emailService
            ...
            .catch((err) =>
              this.logger.error(
                `Error sending email automation: $${err.message}`,
              ),
            );
        } else {
          this.logger.warn(
            `Automation $${auto.name} (SEND_EMAIL) skipped: No contact email found for deal $${payload.dealId}`,
          );
        }
      } else if (auto.actionType === 'SEND_SLACK_MESSAGE') {
        this.logger.log(
          `Slack message dummy action executed for deal $${payload.dealId}`,
        );
      }
    }
  }
