...
export class CurrencyService {
  ...
  async getRates(): Promise<Record<string, number>> {
    ...
    if (
      this.lastFetch &&
      now.getTime() - this.lastFetch.getTime() < 6 * 60 * 60 * 1000
    ) {
      return this.rates;
    }
    ...
    try {
      ...
      if (data && data.rates) {
        ...
        this.lastFetch = now;
        this.logger.log(
          'Currency exchange rates successfully updated from API.',
        );
      }
    } catch (error) {
      this.logger.error(
        'Failed to fetch real-time currency rates. Falling back to static defaults.',
        error,
      );
    }
