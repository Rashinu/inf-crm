...
export class FinanceService {
  ...
  async getSummary(tenantId: string) {
    ...
    for (const p of payments) {
      const amount = await this.currencyService.convertToTRY(
        Number(p.amount),
        p.deal?.currency,
      );
      const paid = await this.currencyService.convertToTRY(
    ...
    for (const d of deals) {
      ...
      const dealAmount = await this.currencyService.convertToTRY(
  ...
  async getCommissions(tenantId: string) {
    ...
    const commissions = await Promise.all(
      deals.map(async (deal) => {
        const amountTry = await this.currencyService.convertToTRY(
