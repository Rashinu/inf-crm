...
export class EmailService {
  ...
  async sendReminder(reminder: any, userEmail: string) {
    ...
    const fromEmail =
      this.configService.get('EMAIL_FROM') || 'INF CRM <noreply@infcrm.com>';
  ...
  async handleCommEmailSend(payload: {
    email: string;
    subject: string;
    htmlContent: string;
  }) {
  ...
  private getSubject(type: string): string {
    switch (type) {
      ...
      case 'DELIVERABLE_DUE':
        return 'Deliverable Due SOON';
      case 'PUBLISH_DATE':
        return 'Publish Date Upcoming';
      default:
        return 'INF CRM Reminder';
    }
  }
  ...
  private getTemplate(type: string) {
    ...
    return (
      templates[type] || ((r) => `<p>Reminder for deal: ${r.deal?.title}</p>`)
    );
  }
}
